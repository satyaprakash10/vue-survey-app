<template>
  <div>
    ...
    <div
      ...
      class="px-6 py-6 overflow-hidden bg-gray-100 divide-y divide-gray-200 rounded-lg shadow-lg sm:divide-y-0 sm:grid sm:grid-cols-2 sm:gap-px"
    >
      <!-- Analysis for attempted survey list -->
      <router-link
        :to="`/${survey.survey_id}`"
        v-for="(survey, index) in surveyStore.userSurveys"
        :key="index"
        class="relative p-6 bg-white border border-gray-300 cursor-pointer hover:shadow-2xl group hover:bg-blue-100 hover:border hover:border-indigo-500 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-500"
      >
        <div>
        ...
        <div class="mt-6">
          <p
            class="mt-2 text-sm font-semibold text-gray-700 hover:text-indigo-500"
          >
            {{ survey.survey_name }}
          </p>
        </div>
